module Rirera
  class Order
    ...
    def get_total_commission
      basic_price = Rirera::BROKERS['broker'][@broker]['basic_price']
      commission_rate = Rirera::BROKERS['broker'][@broker]['commission_rate']
      ...
      [volume_rate_buy, volume_rate_sell].each do |vr|
        if (vr + basic_price) > Rirera::BROKERS['broker'][@broker]['min_rate']
          if (vr + basic_price) > Rirera::BROKERS['broker'][@broker]['max_rate']
            total_commission += Rirera::BROKERS['broker'][@broker]['max_rate']
          else
            total_commission += (basic_price + vr)
          end
        else
          total_commission += Rirera::BROKERS['broker'][@broker]['min_rate']
        end
      end
